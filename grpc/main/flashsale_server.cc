...
public:
  Status flashsale(ServerContext *context, const FlashsaleRequest *request,
    ...
    std::cout << "Flashsale usr_name:" << usr_name << std::endl;
    std::cout << "Flashsale usr_key:" << usr_key << std::endl;
      //连接redis
    ...
    void seckillGoods(std::string usr_name,std::string usr_key,redisContext *redisconn,FlashsaleResponse *response,int repeatCount){
        ++repeatCount;
...
void RunServer() {
    ...
    else
    {
        ...
        MYSQL_ROW row;
        std::cout << "step1" << endl;
        std::cout << "mysql_fetch_row(result)" << mysql_fetch_row(result) << endl;
...
int main(int argc, char *argv[]) {
    ...
    std::cout << "Server listening on " << (file == NULL) << std::endl;
    if (file == NULL)
    {
        return 0;
    }
