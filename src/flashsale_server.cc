...
//    std::string pwd("941320");
//    std::string db_name("flashsale");
...
//redisContext *GetRedisConnection(){
    //开启redis
//    redisContext *redisconn = redisConnect("localhost", 6379);
...
class FlashsaleServiceImpl final : public FlashsaleService::Service {
public:
  Status flashsale(ServerContext *context, const FlashsaleRequest *request,
              FlashsaleResponse *response) override {
    auto username = request->usrname();
    auto userkey = request->usrkey();
    std::cout << "flashsale username : " << username << "userkey : " << userkey <<std::endl;
...
//              //开始抢购
//              flashsaleGoods(usr_name,usr_key,redisconn,response,0);
//          }else{
//              response->set_result("0");
//          }
//      } else {
//          std::cout << "Flashsale failed!!!" <<std::endl;
//          response->set_result("0");
//      }
//
//    redisFree(redisconn);
    ...
    bool usrHasGot(std::string usr_name,std::string usr_key,redisContext *redisconn){
      std::cout << "usrHasGot usr_name : " << usr_name << "usr_key : " << usr_key <<std::endl;
      return true;
...
//        return false;
    ...
    bool checkUserInfo(std::string usr_name,std::string usr_key,redisContext *redisconn){
      std::cout << "checkUserInfo usr_name : " << usr_name << "usr_key : " << usr_key <<std::endl;
       return true;
...
//        return false;
    ...
    void flashsaleGoods(std::string usr_name,std::string usr_key,redisContext *redisconn,FlashsaleResponse *response,int repeatCount){
      std::cout << "flashsaleGoods usr_name : " << usr_name << "usr_key : " << usr_key <<std::endl;
           return;
//        ++repeatCount;
...
//                    std::cout << "Flashsale Succeed!!!" <<std::endl;
...
//                        flashsaleGoods(usr_name,usr_key,redisconn,response,repeatCount);
//                    }else{
//                        response->set_result("0");
//                    }
//                }
...
//
//            response->set_result("0");
//            std::cout << "flashsale failed! The goods has sold out!" << std::endl;
//            return;
//        }
    }
};
...
//
//    //建表
//    std::string creatGoodTableSqlStr("CREATE TABLE IF NOT EXISTS goods_info(g_id VARCHAR(20) NOT NULL, g_name VARCHAR(20) NOT NULL, g_totalcount INT UNSIGNED NOT NULL, PRIMARY KEY (g_id))");
//    mysql_query(connection, creatGoodTableSqlStr.c_str());
//
      //插入商品数据
//    std::string insertGoodsSqlStr("INSERT INTO goods_info(g_id, g_name, g_totalcount) VALUES('10', 'Candy', 50) ON DUPLICATE KEY UPDATE g_id = '10', g_totalcount = 50");
//    mysql_query(connection, insertGoodsSqlStr.c_str());
//
